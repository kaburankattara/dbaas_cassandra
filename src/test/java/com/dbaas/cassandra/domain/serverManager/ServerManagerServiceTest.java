...
public class ServerManagerServiceTest {
	...
	@Autowired
	public ServerManagerServiceTest(ServerManagerService serverManagerService, BaseDao baseDao, UserService userService) {
	...
	public void getAllInstances_ユーザーに紐づくEC2インスタンスが未構築の場合() {
		...
		// テストの実行
		Instances instances = serverManagerService.getAllInstances(user);
	...
	public void deleteServer_複数のインスタンスが存在する中_特定のインスタンスを削除した場合() {
		...
		// テストの実行
		serverManagerService.deleteServer(deleteInstance);
		
		// 検証
		// インスタンスが1以上取得出来ること
		Instances resultInstances = serverManagerService.getInstances(user);
